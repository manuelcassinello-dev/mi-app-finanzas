...
# 1. Configuración de página y Estilo Pro
...
# 2. URLs de Google Sheets (Tus IDs reales)
...
# 3. Función Maestra de Carga (Limpia números y fechas)
...
with tab_p:
    ...
    df_m = cargar_datos(GIDS["pers_mov"])
    df_i = cargar_datos(GIDS["pers_inv"])

    # 🟢 SECCIÓN INVERSIONES (Métricas verdes)
    if not df_i.empty:
    ...
    # 📈 SECCIÓN EVOLUCIÓN (La línea que habías perdido)
    if not df_m.empty:
        st.subheader("📊
